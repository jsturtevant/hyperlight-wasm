    ...
    match vt {
        ...
        Value::Flags(ns) => {
            ...
            if s.is_wasmtime_guest {
                // The wasmtime flags! macro generates named constants (e.g. FLAGS::READ).
                // Build the value by OR-ing constants for each set bit.
...
pub fn emit_hl_marshal_toplevel_value(
    ...
    match vt {
        ...
        Value::Flags(ns) => {
            let bytes = usize::div_ceil(ns.len(), 8);
            if s.is_wasmtime_guest {
                // wasmtime flags! macro types: use as_array() to get u32 values
                quote! {
                    let arr = #id.as_array();
                    let mut bytes = alloc::vec![0u8; #bytes];
